  ...
  return (
    <article className={`poke-card border-${pokemon?.types[0].type.name}`} onClick={handleClick}>
      ...
      <section className='poke-card__body'>
        ...
        <ul className='poke-card__types-container'>{pokemon?.types.map(type =>
          (<li className='poke-card__type' key={type.type.name}>{type.type.name}</li>))}</ul>
      </section>

      <footer className='poke-card__footer'>
        <ul className='poke-card__stats-container'>{pokemon?.stats.map(stat =>
